function Validator(options) {
            ...
            if (isFormValid) {
                ...
                if (typeof options.onSubmit === 'function') {
                    ...
                    var formValues = Array.from(enableInputs).reduce(function (values, input) {
                        switch(input.type) {
                            case 'radio':
                                values[input.name] = input.formElement.querySelector('input.[name="' + input.name + '"]:checked').value;
                                break;
                            case 'checkbox':
    ...
    // Xử lý lăp qua mỗi rule và xử lý (lắng nghe sự kiện blur, input)
    options.rules.forEach((rule) => {
